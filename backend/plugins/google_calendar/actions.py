...
def create_event(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            success=True,
            data={"event_id": event["id"], "url": url},
            display_message=f"Created event '{title}'",
            resource_url=url,
            error_message=None,
        )
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to create event: {e}"
        )
...
def list_events(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            success=True,
            data={"events": summaries, "total": len(summaries)},
            display_message=f"Found {len(summaries)} events",
            resource_url=None,
            error_message=None,
        )
    except Exception as e:
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to list events: {e}"
        )
...
def update_event(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to update event: {e}"
        )
