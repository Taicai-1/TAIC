...
def create_sheet(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        if sheets_data:
            body["sheets"] = [
                {"properties": {"title": s.get("title") or s.get("name") or f"Sheet{i + 1}"}}
        ...
        return ActionResult(
            success=True,
            data={"spreadsheet_id": ss_id, "url": url},
            display_message=f"Created spreadsheet '{title}'",
            resource_url=url,
            error_message=None,
        )
...
def update_sheet(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            success=True,
            data={"spreadsheet_id": ss_id},
            display_message="Updated spreadsheet",
            resource_url=url,
            error_message=None,
        )
    except Exception as e:
        ...
        return ActionResult(
            ...
            display_message="",
            resource_url=None,
            error_message=f"Failed to update spreadsheet: {e}",
        )
...
def read_sheet(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            ...
            display_message=f"Read {len(values)} rows from spreadsheet",
            resource_url=None,
            error_message=None,
        )
    except Exception as e:
        ...
        return ActionResult(
            ...
            display_message="",
            resource_url=None,
            error_message=f"Failed to read spreadsheet: {e}",
        )
