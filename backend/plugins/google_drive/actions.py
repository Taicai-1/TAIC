...
def create_folder(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            success=True,
            data={"folder_id": folder["id"], "url": url},
            display_message=f"Created folder '{name}'",
            resource_url=url,
            error_message=None,
        )
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to create folder: {e}"
        )
...
def move_file(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            ...
            display_message="Moved file to folder",
            resource_url=None,
            error_message=None,
        )
    except Exception as e:
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to move file: {e}"
        )
...
def share_file(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            success=True,
            data={"file_id": file_id, "shared_with": email, "role": role},
            display_message=f"Shared file with {email} as {role}",
            resource_url=None,
            error_message=None,
        )
    except Exception as e:
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to share file: {e}"
        )
...
def search_files(args: dict, credentials) -> ActionResult:
    ...
    try:
        ...
        results = (
            service.files()
            .list(q=query, pageSize=max_results, fields="files(id,name,mimeType,webViewLink,modifiedTime)")
            .execute()
        )
        ...
        return ActionResult(
            success=True,
            data={"files": summaries, "total": len(summaries)},
            display_message=f"Found {len(summaries)} files matching '{query}'",
            resource_url=None,
            error_message=None,
        )
    except Exception as e:
        ...
        return ActionResult(
            success=False, data={}, display_message="", resource_url=None, error_message=f"Failed to search files: {e}"
        )
