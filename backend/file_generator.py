...
class FileGenerator:
    ...
    def _format_answer_with_table(self, answer: str, table_data: List[List[str]]) -> str:
        ...
        for row in table_data:
            ...
            patterns_to_remove = [f"{row[0]}: {row[1]}", f"- {row[0]}: {row[1]}", f"{row[0]}: {row[1]}"]
    ...
    def generate_pdf(self, title: str, content: str, table_data: List[List[str]] = None) -> io.BytesIO:
        ...
        title_style = ParagraphStyle(
            "CustomTitle", parent=self.styles["Heading1"], fontSize=16, spaceAfter=30, textColor=colors.darkblue
        )
