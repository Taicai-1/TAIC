...
async def test_shared_agent_accessible(client, db_session, test_user, test_user_token):
    ...
    share = AgentShareFactory.build(agent_id=shared_agent.id, user_id=test_user.id, can_edit=False)
    db_session.add(share)
    db_session.flush()

    # test_user should be able to access the shared agent
    response = await client.get(f"/agents/{shared_agent.id}", cookies={"token": test_user_token})
...
async def test_conversation_owner_isolation(client, db_session, test_user, test_agent, test_user_token):
    ...
    other_conversation = ConversationFactory.build(agent_id=other_agent.id, user_id=other_user.id)
    db_session.add(other_conversation)
    db_session.flush()

    # test_user tries to access other_user's conversation messages
    response = await client.get(f"/conversations/{other_conversation.id}/messages", cookies={"token": test_user_token})
...
async def test_document_delete_owner_only(client, db_session, test_user, test_agent, test_user_token):
    ...
    other_document = DocumentFactory.build(user_id=other_user.id, agent_id=other_agent.id)
    db_session.add(other_document)
    db_session.flush()

    # test_user tries to delete other_user's document
    response = await client.delete(f"/documents/{other_document.id}", cookies={"token": test_user_token})
...
async def test_unauthenticated_access_blocked(client, db_session, test_agent):
    ...
    response = await client.post("/agents", data={"name": "Unauthorized Agent", "type": "conversationnel"})
    assert response.status_code == 401
